...
def get_all_sequences_once(sequences_cache: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"  Fetched {len(all_sequences)} sequences from Apollo")
...
def get_sequence_name_from_cache(sequence_id: str, sequences_cache: Dict[str, Any]) -> Optional[str]:
    """Reverse lookup sequence name from ID using cache."""
    if 'all_sequences' in sequences_cache:
        for seq in sequences_cache['all_sequences']:
            if seq.get('id') == sequence_id:
                return seq.get('name')

    for cache_key, cached_id in sequences_cache.items():
        if cache_key != 'all_sequences' and cached_id == sequence_id:
...
def activate_sequence(sequence_id: str, sequence_name: str = None) -> Dict[str, Any]:
...
def process_csv(csv_file_path: str):
    ...
    stats = {
        ...
        'errors': 0,
        'used_sequences': set(),      # Track unique sequence IDs that received contacts
        'activation_results': {}      # Track activation success/failure per sequence
    }
    ...
    with open(csv_file_path, 'r', encoding='iso-8859-1') as csvfile:
        ...
        for row in reader:
            ...
            if result['success']:
                stats['added'] += 1
                stats['used_sequences'].add(sequence_id)  # Track this sequence was used
    ...
    # Activate all sequences that received contacts
    if stats['used_sequences']:
        print("\n" + "="*50)
        print("ACTIVATING SEQUENCES")
        print("="*50)
        print(f"Activating {len(stats['used_sequences'])} sequence(s)\n")

        for i, sequence_id in enumerate(stats['used_sequences'], 1):
            sequence_name = get_sequence_name_from_cache(sequence_id, sequences_cache)
            print(f"[{i}/{len(stats['used_sequences'])}] {sequence_name or sequence_id}")

            result = activate_sequence(sequence_id, sequence_name)
            stats['activation_results'][sequence_id] = result
        ...
        successful = sum(1 for r in stats['activation_results'].values() if r['success'])
        failed = len(stats['activation_results']) - successful

        print(f"\nActivation: {successful} successful, {failed} failed")
    ...
    if stats['used_sequences']:
        print(f"\nSequences used: {len(stats['used_sequences'])}")
        if stats['activation_results']:
            successful = sum(1 for r in stats['activation_results'].values() if r['success'])
            failed = len(stats['activation_results']) - successful
            print(f"  Activated: {successful}")
            if failed > 0:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python3 add_to_apollo_sequences.py /path/to/file.csv")
        sys.exit(1)
